    ...
    if (added.length > 0) {
      ...
      added.forEach(function(email) {
        try {
          GmailApp.sendEmail(
            email,
            'You\'ve been added as a collaborator — ' + (session.name || 'Learning Library Session'),
            '',
      ...
      '<body style="margin:0;padding:0;background:#f5f7fa;font-family:\'Open Sans\',Arial,sans-serif;">' +
        '<div style="max-width:560px;margin:24px auto;background:#fff;border-radius:8px;overflow:hidden;">' +
          '<div style="background:' + navy + ';padding:24px 28px;">' +
            '<p style="color:' + cyan + ';font-size:11px;letter-spacing:2px;text-transform:uppercase;margin:0 0 5px;">Learning Library Learning Community</p>' +
          ...
          '<div style="background:#f5f7fa;padding:16px 28px;border-top:1px solid #e0e6ed;">' +
            '<p style="color:#8a9bae;font-size:12px;margin:0;">Learning Library Learning Community &nbsp;·&nbsp; ' + (session.date || '') + '</p>' +
          '</div>' +
        '</div>' +
      '</body></html>';
  }
